    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("系统切换浅色/深色时,终端配色实时跟随;claude 会话以 auto 主题启动(通过 --settings 注入,仅对 Leader 启动的会话生效,不改动全局 ~/.claude 设置),kimi 会话读取其 tui.toml 的 theme=\"auto\" —— 两者都订阅终端配色通知(DEC 2031),切换外观时无需重开即可实时换主题。关闭则终端固定 Kaku Dark、claude 固定深色主题。")
                    .font(.caption2).foregroundStyle(.tertiary).fixedSize(horizontal: false, vertical: true)
